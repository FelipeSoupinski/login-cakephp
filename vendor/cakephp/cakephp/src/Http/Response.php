...
class Response implements ResponseInterface
{
    ...
    protected function validateFile($path)
    {
        if (strpos($path, '../') !== false || strpos($path, '..\\') !== false) {
            throw new NotFoundException(__d('cake', 'O arquivo solicitado contém `..` e não será lido.'));
        }
        if (!is_file($path)) {
            deprecationWarning(
        ...
        if (!$file->exists() || !$file->readable()) {
            ...
            throw new NotFoundException(__d('cake', 'O arquivo solicitado não foi encontrado'));
        }
